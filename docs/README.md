# Documentation

This directory contains detailed, command-level documentation for the video-generator CLI.

## Command Docs

- [Global CLI options](commands/global-options.md)
- [from-text command](commands/from-text.md)
- [from-audio command](commands/from-audio.md)
- [transcribe command](commands/transcribe.md)
- [doctor command](commands/doctor.md)

## How to use these docs

1. Start with Global CLI options to understand flags that apply to every command.
2. Pick the command page for your workflow.
3. Use the flowchart on each page to understand what happens internally.
4. Use the examples as templates and adjust file paths and prompts.
